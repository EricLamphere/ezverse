#' Conflicts between the ezverse and other packages
#'
#' This function lists all the conflicts between packages in the ezverse
...
ezverse_conflicts <- function() {
  ...
  tidy_names <- paste0("package:", ezverse_packages())
  conflicts <- purrr::keep(conflicts, ~ any(.x %in% tidy_names))

  conflict_funs <- purrr::imap(conflicts, confirm_conflict)
  conflict_funs <- purrr::compact(conflict_funs)

  structure(conflict_funs, class = "ezverse_conflicts")
}

ezverse_conflict_message <- function(x) {
  if (length(x) == 0) return("")

  header <- cli::rule(
    left = crayon::bold("Conflicts"),
    right = "ezverse_conflicts()"
  )
...
#' @export
print.ezverse_conflicts <- function(x, ..., startup = FALSE) {
  cli::cat_line(ezverse_conflict_message(x))
}
